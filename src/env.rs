    ...
    pub fn new() -> Self {
        Self {
            ...
            start_time: env::var("START_TIME")
                ...
                .parse::<DateTime<Utc>>()
                .expect("START_TIME is not valid"),
            mid_evals_time: env::var("MID_EVALS_TIME")
                .expect("MID_EVALS_TIME is not set")
                .parse::<DateTime<Utc>>()
                .expect("MID_EVALS_TIME is not valid"),
            end_evals_time: env::var("END_EVALS_TIME")
                .expect("END_EVALS_TIME is not set")
                .parse::<DateTime<Utc>>()
                .expect("END_EVALS_TIME is not valid"),
            ...
            db_port: env::var("DATABASE_PORT")
                .expect("DATABASE_PORT is not set")
                .parse::<u16>()
                .expect("DATABASE_PORT is not valid"),
            db_name: env::var("DATABASE_NAME").expect("DATABASE_NAME is not set"),
            mid_evals_ended: env::var("MID_EVALS_ENDED")
                .expect("MID_EVALS_ENDED is not set")
                .parse::<bool>()
                .expect("MID_EVALS_ENDED is not valid"),
